...
class CartItemOptionsProcessor
{
     ...
     */
    public function __construct(\Magento\Quote\Model\Quote\Item\CartItemProcessorsPool $cartItemProcessorsPool)
    {
     ...
     * @return \Magento\Framework\DataObject|float
     */
    public function addCustomOptionsToBuyRequest(CartItemInterface $cartItem, $params)
    {
    ...
    public function applyImage($image,CartItemInterface $cartItem)
    {
        if (isset($this->cartItemProcessors[$image])) {
            $cartItem = $this->cartItemProcessors[$image]->processOptions($cartItem);
        }
        return $cartItem;
    }
